  ...
  */
  {
    ...
    std::wstring line = L"--- Server is shutting down...";
...
void Server::start(unsigned short port)
{
  ...
  actionstream<<"This is whiskers75's custom server ;)"<<std::endl;
}
      ...
      if(command == TOSERVER_INIT2)
	{
	  ...
	  */
	  ...
	  if(getClient(peer_id)->net_proto_version < LATEST_PROTOCOL_VERSION)
	    {
	      SendChatMessage(peer_id, L"*** WARNING: YOUR CLIENT'S "
      ...
      else if(command == TOSERVER_CHAT_MESSAGE)
	{
	  ...
	  */
	  ...
	  if(message[0] == L'/')
		...
		line += L"-!- You do not have permission to chat.";
		send_to_sender = true;
	      }
	    }
      ...
      else if(command == TOSERVER_INTERACT)
	{
	  ...
	  */
	  if(action == 0 || action == 2 || action == 3)
		...
		RemoteClient *client = getClient(peer_id);
                SendChatMessage(peer_id, L"*** Cheat alert! You tried to interact with something that you can't reach!");
