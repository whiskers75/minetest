  ...
  try{
	...
	*/
	...
	delete[] text;
      ...
      {
	wchar_t* text = wgettext("Hacking at server...");
	...
	while(device->run())
	  {
	    ...
	    // Break conditions
	    if(!client.connectedAndInitialized()){
	      error_message = L"Disconnected!";
	      errorstream<<wide_to_narrow(error_message)<<std::endl;
	      break;
	    }
	    if(input->wasKeyDown(EscapeKey)){
	      content_aborted = true;
	      infostream<<"Aborted [Escape]"<<std::endl;
	      break;
	    }
